...
test("local data and the generated workbook are present", async () => {
  ...
  assert.equal(state.finalPlan.dates, "2026年8月28日–30日（周五–周日）");
  assert.equal(state.finalPlan.nights, 3);
  ...
  assert.match(state.finalPlan.summary, /每天约9点出门/);
  assert.match(state.finalPlan.summary, /晚上优先回民宿一起做饭休息/);
  assert.match(state.finalPlan.summary, /30日不安排南线/);
  assert.ok(state.finalPlan.itinerary.some((item) => item.title === "牛岛骑行 / 环岛巴士（二选一）"));
  assert.ok(state.finalPlan.itinerary.every((item) => item.time >= "09:00"));
  assert.ok(state.finalPlan.itinerary.every((item) => !/柱状节理|正房瀑布|泰迪熊|凯蒂猫/.test(item.title)));
