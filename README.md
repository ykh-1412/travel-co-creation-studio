...
一个可部署在个人电脑或服务器上的团队旅行资料整理网站。朋友提交攻略、住宿、餐饮、密室、室内休闲或景点链接后，后台读取网页并调用 DeepSeek（也可切换豆包、Ollama 或演示模式），先识别大类、子分类和特征标签，再把专属字段写入 Excel，最后由网站展示候选和周末行程。
...
- `行程首页`、`候选决策台`、六张分类明细、`处理报告`、`预订清单`、`两日行程` 等 14 张工作表
